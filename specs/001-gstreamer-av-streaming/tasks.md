...
- [X] T018 [US1] 已删除 `crates/ipcam-rtsp/src/lib.rs` 的 `play_loop` 与 `crates/ipcam-rtsp/src/rtp.rs`（保留 connect/teardown/sdp/RtspError 供 probe 的 connect 阶段），`tests/rtsp_e2e.rs` 的 play_loop 用例已删、保留 connect/SDP/Digest 用例；`web-display` 不再依赖 ipcam-rtsp。**备注：用户确认提前执行（未等实机验证），实机回退靠 git revert**
